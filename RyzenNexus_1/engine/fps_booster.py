    ...
    for proc in psutil.process_iter(["pid", "name"]):
        try:
            if proc.info["name"].lower() in BACKGROUND_HOGS:
                ...
                success, msg = set_high_priority_and_affinity(
                    proc.info["pid"], is_game=False
                )
                if success:
                    throttled_count += 1
        except:
...
def apply_dcie_isolation(quarantine_mask):
    """Innovation 1: DCIE - Constraints background workloads to reduce context-switch overhead."""
    import psutil
    import os

    for proc in psutil.process_iter(["name"]):
        try:
            # If it's not the game and not this script, move it to quarantine
            if (
                not process_scanner.is_game_running(proc.info["name"])
                and proc.pid != os.getpid()
            ):
                p = psutil.Process(proc.pid)
                p.cpu_affinity(quarantine_mask)
